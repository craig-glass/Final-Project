...
public class AdjustRoadHeight : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < vertices.Length - 1; i++)
        {
            ...
            UnityEngine.Debug.Log("vertices[" + i + "] = " + vertices[i]);
            Vector3 vert = vertices[i];

            vert += transform.position + transform.lossyScale;
            UnityEngine.Debug.Log("vert = " + vert);

            UnityEngine.Debug.Log(Terrain.activeTerrain.SampleHeight(vert));
            vert.y = Terrain.activeTerrain.SampleHeight(vert);

            vert = transform.InverseTransformPoint(vert);
            UnityEngine.Debug.Log("new vert = " + vert);
